...
def analyze_articles(
    ...
    category_tally = {category: 0 for category in CATEGORY_LABELS}
    inconsistency_links: list[str] = []
    articles: list[dict] = []
    ...
    for index, pmid in enumerate(pmids, start=1):
        ...
        article_record = {
            ...
            "pmcid": pmcid,
            "article_url": PMC_ARTICLE_URL.format(pmcid=pmcid) if pmcid else None,
            "status": "missing_pmcid" if not pmcid else "pending",
            "fetch_error": None,
            "summary_tables": 0,
            "outcomes": [],
        }
        ...
        try:
            article_html = fetch_article_html(session, article_record["article_url"], pause_seconds)
        except RuntimeError as exc:
            article_record["status"] = "fetch_failed"
            article_record["fetch_error"] = str(exc)
            print(
                f"[{index}/{len(pmids)}] {pmid} -> {pmcid} fetch_failed",
                file=sys.stderr,
            )
            articles.append(article_record)
            continue
        article_record["title"] = extract_article_title(article_html, article_record["title"])

        if is_protocol_article(article_html):
            article_record["status"] = "protocol_skipped"
            print(
                f"[{index}/{len(pmids)}] {pmid} -> {pmcid} protocol_skipped",
                file=sys.stderr,
            )
            articles.append(article_record)
            continue
        ...
        article_record["summary_tables"] = len(summary_sections)
        article_record["status"] = "review_processed"

        all_outcomes: list[dict] = []
        article_categories: set[str] = set()
        for section_title, section_html in summary_sections:
            all_outcomes.extend(analyze_summary_section(section_title, section_html))

        article_record["outcomes"] = all_outcomes

        for outcome in all_outcomes:
            for category in outcome["downgrade_categories"]:
                category_tally[category] += 1
                article_categories.add(category)

        if "inconsistency" in article_categories:
            inconsistency_links.append(article_record["article_url"])

        articles.append(article_record)
        print(
            f"[{index}/{len(pmids)}] {pmid} -> {pmcid} "
            f"{article_record['status']} tables={article_record['summary_tables']} "
            f"outcomes={len(all_outcomes)}",
            file=sys.stderr,
        )

    return {
        "category_tally": category_tally,
        "inconsistency_links": sorted(set(inconsistency_links)),
        "articles": articles,
    }
...
def build_summary_text(
    ...
    processed_reviews = [a for a in articles if a["status"] == "review_processed"]
    protocol_skips = [a for a in articles if a["status"] == "protocol_skipped"]
    missing_pmcids = [a for a in articles if a["status"] == "missing_pmcid"]
    fetch_failures = [a for a in articles if a["status"] == "fetch_failed"]
    reviews_with_outcomes = [a for a in processed_reviews if a["outcomes"]]

    lines = [
        "GRADE downgrade summary for 2025 open-access Cochrane reviews",
        f"PubMed query returned {total_query_results} records total.",
        f"Analyzed the first {len(selected_pmids)} PubMed IDs returned by the query.",
        f"Completed reviews processed: {len(processed_reviews)}",
        f"Protocols skipped: {len(protocol_skips)}",
        f"Records missing a PMCID: {len(missing_pmcids)}",
        f"PMC fetch failures: {len(fetch_failures)}",
        f"Completed reviews with parsed Summary of findings outcomes: {len(reviews_with_outcomes)}",
        "",
        "Downgrade tally by GRADE category:",
    ]

    for category, label in CATEGORY_LABELS.items():
        lines.append(f"- {label}: {analysis['category_tally'][category]}")

    lines.extend(
        [
            "",
            "PMC links with at least one inconsistency downgrade:",
        ]
    )
    if analysis["inconsistency_links"]:
        lines.extend(f"- {link}" for link in analysis["inconsistency_links"])
    else:
        lines.append("- None")
    ...
    for article in articles:
        lines.append(
            "- "
            f"PMID {article['pmid']} | PMCID {article.get('pmcid') or 'N/A'} | "
            f"{article['status']} | tables={article['summary_tables']} | "
            f"outcomes={len(article['outcomes'])} | {article['title']}"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Query PubMed for 2025 open-access Cochrane reviews, extract Summary "
            "of findings downgrade reasons from PMC, and tally GRADE categories."
        )
    )
    parser.add_argument("--limit", type=int, default=50, help="Number of PubMed IDs to process.")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path.cwd(),
        help="Directory where JSON and text outputs will be written.",
    )
    parser.add_argument(
        "--pause-seconds",
        type=float,
        default=0.8,
        help="Delay inserted after each PMC article fetch.",
    )
    ...
    pmids, total_query_results = search_pubmed(session, args.limit)
    ...
    payload = {
        "query": PUBMED_QUERY,
        "total_query_results": total_query_results,
        "selected_pmids": pmids,
        "category_labels": CATEGORY_LABELS,
        "analysis": analysis,
    }
    json_path = args.output_dir / "grade_downgrade_results.json"
    text_path = args.output_dir / "grade_downgrade_summary.txt"
    inconsistency_path = args.output_dir / "inconsistency_pmc_links.txt"

    json_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    summary_text = build_summary_text(total_query_results, pmids, analysis)
    text_path.write_text(summary_text + "\n", encoding="utf-8")
    inconsistency_lines = analysis["inconsistency_links"] or ["None"]
    inconsistency_path.write_text("\n".join(inconsistency_lines) + "\n", encoding="utf-8")
